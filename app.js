...
function resize(){
  ...
  if(!wrap){W=window.innerWidth;H=window.innerHeight;}
  else{W=wrap.offsetWidth;H=wrap.offsetHeight;}
...
//  SELECTION & AUDIO
// ═══════════════════════════════
window.ccPick=function(key,btn){
  ...
  document.getElementById('cc-ebar').style.background=`linear-gradient(90deg,rgba(255,255,255,.7),${d.hex})`;
  document.getElementById('cc-btn').style.color=d.hex;
  document.querySelectorAll('.cc-btn').forEach(b=>b.classList.remove('on'));
...
function loop(){
  ...
  document.getElementById('cc-ebar').style.width=`${energy}%`;
...
window.ccStartC=function(e){
  if(e)e.preventDefault();
  if(!audioOn)initAudio();
  charging=true;document.getElementById('cc-btn').classList.add('on');
};
window.ccStopC=function(e){
  if(e)e.preventDefault();
  charging=false;document.getElementById('cc-btn').classList.remove('on');
};
